...
def fetch_wikimedia_real_hd_photo(query_terms: list, out_path: Path) -> bool:
    """Busca FOTOS REAIS EM ALTA DEFINIÇÃO (1280px) via Wikimedia Commons API."""
                            ...
                            with urllib.request.urlopen(img_req, timeout=12) as img_resp:
                                ...
                                if len(content) > 10000:
                                    with open(out_path, 'wb') as f:
                                        f.write(content)
                                    print(f"    ✓ Foto REAL HD Obtida: '{term}' ({out_path.name})")
                                    return True
        except Exception:
            pass
    return False
...
def format_photo_to_916_hd(raw_img_path: Path, out_path: Path):
    """Formata foto real para 9:16 HD 1080x1920 com crop perfeito e tratamento fotográfico."""
    w, h = 1080, 1920
    ...
    try:
        ...
        # Tratamento fotográfico documental
...
def render_real_photo_scene_clip(img_path: Path, scene_id: int, narration_text: str, duration: float, movement_type: str, out_mp4_path: Path):
    """
    Renderiza o clipe MP4 com a FOTO REAL DOCUMENTAL HD em movimento 24 FPS,
    ...
    out_v.release()
    print(f"    ✓ [FOTO REAL CLIP OK] Cena {scene_id} ({movement_type.upper()}) renderizada ({duration:.1f}s)")


REAL_DOCUMENTARY_SCENES = [
    # CENA 1 — FOTO REAL DA SERRA DO ARARIPE NO SERTÃO DO CEARÁ
    {
        "scene_id": 1,
    ...
    # CENA 2 — FOTO REAL DA COMUNIDADE RURAL E SERTANEJOS NO CEARÁ
    {
        "scene_id": 2,
        ...
        "duration": 9.0,
        "queries": ["Juazeiro do Norte Padre Cicero statue", "Comunidade rural Ceara sertao", "Sertao Ceara farm dam water"]
    },
    # CENA 3 — FOTO REAL DOS TRABALHADORES E LAVOURAS NO SERTÃO
    {
        "scene_id": 3,
        ...
        "duration": 8.5,
        "queries": ["Agricultura sertao Ceara", "Sertao farm Brazil historical", "Feira sertaneja Ceara"]
    },
    # CENA 4 — FOTO REAL DE FAZENDA HISTÓRICA E CAATINGA NO CEARÁ
    {
        "scene_id": 4,
    ...
    # CENA 5 — FOTO REAL HISTÓRICA DE BIPLANOS MILITARES DOS ANOS 1930
    {
        "scene_id": 5,
    ...
    # CENA 6 — FOTO REAL DO PÔR DO SOL NA CHAPADA DO ARARIPE
    {
        ...
        "duration": 8.0,
        "queries": ["Chapada do Araripe sunset Ceara", "Serra do Araripe sunset landscape", "Pôr do sol Ceara sertão"]
    }
]
...
def produce_real_documentary_caldeirao_video():
    topic_id = "misterio_caldeirao_do_deserto"
    print(f"\n==========================================")
    print(f"[RE-RENDER COM FOTOS REAIS DOCUMENTAIS EM ALTA DEFINIÇÃO]")
    print(f"==========================================")
    ...
    # Deletar arquivos antigos para garantir substituição 100% limpa
    ...
    for sc in REAL_DOCUMENTARY_SCENES:
        ...
        # 1. Buscar Foto REAL HD
        fetch_wikimedia_real_hd_photo(queries, raw_img_path)
        format_photo_to_916_hd(raw_img_path, formatted_img_path)

        # 2. Renderizar Clipe 24 FPS
        render_real_photo_scene_clip(formatted_img_path, scene_id, narration, dur, movement_type, scene_mp4)
        ...
        print(f"    [SCENE FOTO REAL OK] Cena {scene_id}/6 masterizada ({voice_dur:.2f}s | Total: {current_time:.1f}s)")
    ...
    temp_audio_file = str(output_dir / "temp_audio_real_photos.m4a")
    ...
    print(f"\n  🎉 [RE-RENDERIZAÇÃO FOTOS REAIS CONCLUÍDA] VÍDEO COMPLETO ({current_time:.1f}s): {master_path}")
    return master_path


def main():
    produce_real_documentary_caldeirao_video()
