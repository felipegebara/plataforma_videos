...
SHORTS_FULOZINHA = [
    {
        ...
        "title": "Visão Histórica",
        "hook_text": "A ORIGEM REAL DE COMADRE FULOZINHA 📜",
        "sub_text": "ZONA DA MATA NORDESTINA",
        "style_prefix": "Photorealistic 8k National Geographic documentary photograph, ARRI Alexa 65, historical 19th century Brazil, Zona da Mata Pernambuco, sugarcane plantation forest, atmospheric volumetric lighting, no human faces, RAW",
        "scenes": [
            {
                "scene_id": 1,
            ...
            },
            {
                ...
                "prompt": "Hyperrealistic 8k macro photograph of ancient mossy tree trunk in Brazilian rainforest glowing with subtle golden light, natural forest environment, no human faces"
            },
            {
                "scene_id": 4,
                "movement_type": "macro_pan",
                "narration": "Caçadores ofereciam fumo de rolo e mel nas árvores para pedir permissão e evitar se perder na mata.",
                "prompt": "Macro 8k shot of traditional roll tobacco and small clay pot of honey placed respectfully at the root of a massive banyan tree, cinematic dark forest lighting"
            },
            {
                "scene_id": 5,
    ...
    {
        "topic_id": "short_comadre_fulozinha_folclore",
        "title": "Visão Folclórica",
        "hook_text": "O MISTÉRIO DO ASSOBIO DA MATA! 🌿👻",
        "sub_text": "FOLCLORE NORDESTINO",
        "style_prefix": "Unreal Engine 5 render, hyperrealistic 3D cinematic masterpiece, Octane render, ray tracing global illumination, Hollywood blockbuster VFX, dark magical forest night, no human faces, 8k",
        "scenes": [
            {
                "scene_id": 1,
                "movement_type": "drone_reveal",
                "narration": "Se você ouvir um assobio fino que parece vir de longe, cuidado: a Comadre Fulozinha está bem do seu lado!",
                "prompt": "Epic 3D render of dark enchanting forest at night, glowing blue fireflies floating in air, full moon lighting up tall trees, volumetric fog, no human faces"
            },
            {
                "scene_id": 2,
                "movement_type": "slow_push_in",
                "narration": "Dizem que ela é uma cabocla misteriosa com cabelos longos que cobrem o corpo e dão nós na crina dos cavalos.",
                "prompt": "Hyperdetailed 3D shot of wild horse standing under moonlight in forest clearing with braided mane, glowing mystical particles around, no human faces"
            },
            {
                "scene_id": 3,
                "movement_type": "quick_pan_left",
                "narration": "Quando fica zangada, ela usa cipós para trançar os cabelos dos invasores e chicotear quem maltrata a natureza.",
                "prompt": "Cinematic 3D render of twisted jungle vines moving around ancient trees, mystical green magic energy, dark forest background, no human faces"
            },
            {
                "scene_id": 4,
                "movement_type": "dolly_forward",
                "narration": "Para acalmá-la, os sertanejos deixavam oferendas doces escondidas nas cavidades dos troncos velhos.",
                "prompt": "Beautiful 3D render of small wooden bowl with wild honey and flowers inside a hollow ancient tree trunk glowing under moonlight, no human faces"
            },
            {
                "scene_id": 5,
                "movement_type": "macro_pan",
                "narration": "Seu assobio engana os sentidos: quando parece perto está longe, e quando parece longe ela está atrás de você!",
                "prompt": "Mystical 3D shot of swirling wind currents through dark forest trees at midnight, ethereal blue light trails, cinematic atmospheric VFX, no human faces"
            },
            {
                "scene_id": 6,
                "movement_type": "slow_zoom_out",
                "narration": "Você teria coragem de entrar na mata à noite? Comente e siga o Rota Calculada!",
                "prompt": "Breathtaking 3D wide shot of misty forest valley illuminated by glowing full moon and stars, IMAX blockbuster visual, no human faces"
            }
        ]
    }
]
...
def fetch_ai_image(prompt: str, style_prefix: str, seed_id: int, out_path: Path) -> bool:
    ...
    seed_val = (int(time.time()) + seed_id * 5555) % 999999
            ...
            with urllib.request.urlopen(req, timeout=18) as resp:
                ...
                if len(content) > 15000:
                    with open(out_path, "wb") as f:
                        f.write(content)
                    print(f"  ✓ Imagem gerada ({out_path.name})")
                    return True
        except Exception:
            time.sleep(1.0)
    return False
...
def render_scene(img_path: Path, scene_id: int, dur: float, movement: str, banner: str, sub: str, out_mp4: Path):
    ...
    for f in range(total_frames):
        ...
        else: # drone_reveal & slow_zoom_out
...
def produce_comadre_fulozinha_shorts():
    ...
    print("==========================================")
    print("[PRODUZINDO 2 SHORTS DA COMADRE FULOZINHA: 6 IMAGENS E ~25s]")
    print("==========================================")

    for s_idx, spec in enumerate(SHORTS_FULOZINHA, 1):
        topic_id = spec["topic_id"]
        title_txt = spec["title"]
        hook = spec["hook_text"]
        sub = spec["sub_text"]
        prefix = spec["style_prefix"]
        scenes = spec["scenes"]
        ...
        for sc in scenes:
            ...
            fetch_ai_image(prompt, prefix, s_idx * 100 + sc_id, raw_p)
        ...
        print(f"🎉 [{title_txt.upper()} CONCLUÍDO] Duração: {current_time:.1f}s -> {master_p}")

if __name__ == "__main__":
    produce_comadre_fulozinha_shorts()
