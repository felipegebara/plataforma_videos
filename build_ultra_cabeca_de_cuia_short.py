...
def fetch_cabeca_de_cuia_ai_image_8k(prompt: str, scene_id: int, out_path: Path) -> bool:
    """Gera imagens de IA 8K fotorealistas de alta fidelidade para o Cabeça de Cuia."""
    ...
    seed_val = (int(time.time()) + scene_id * 7777) % 999999
            ...
            with urllib.request.urlopen(req, timeout=18) as resp:
                ...
                if len(content) > 15000:
                    with open(out_path, "wb") as f:
                        f.write(content)
                    print(f"    ✓ [IMAGEM IA 8K CABEÇA DE CUIA GERADA] Cena {scene_id}: '{prompt[:45]}...' ({out_path.name})")
                    return True
        except Exception:
            time.sleep(1.0)
    return False
...
def format_photo_to_916_hd(raw_img_path: Path, out_path: Path):
    """Formata imagem para 9:16 HD 1080x1920 com tratamento cinematográfico."""
    w, h = 1080, 1920
...
def render_fast_dynamic_scene_clip(img_path: Path, scene_id: int, duration: float, movement_type: str, out_mp4_path: Path):
    ...
    mantendo visual limpo (sem legendas sobrepostas) e moldura cinematográfica dourada.
    """
    ...
    for f_idx in range(total_frames):
        ...
        # Movimentos de Câmera RÁPIDOS & IMPACTANTES para Shorts de 12-20s
        ...
        # HOOK DE IMPACTO APENAS NA CENA 1 (Primeiros 2 Segundos)
        if scene_id == 1 and f_idx < int(2.5 * fps):
            ...
            draw.text((540, 320), "O MONSTRO DO RIO PARNAÍBA!", fill=(255, 215, 0), font=font_hook, anchor="mm", stroke_width=4, stroke_fill=(0, 0, 0))
            draw.text((540, 400), "A LENDÁRIA PRAGA DO PIAUÍ", fill=(255, 255, 255), font=font_hook, anchor="mm", stroke_width=4, stroke_fill=(0, 0, 0))
...
# ESTRUTURA VIRAL DO SHORT CABEÇA DE CUIA (DURAÇÃO TOTAL: 16 SEGUNTOS — REGRA ROTA CALCULADA)
CABECA_DE_CUIA_SHORT_SCENES = [
    # CENA 1: HOOK POLÊMICO (3.0s)
    {
        "scene_id": 1,
        "movement_type": "fast_push_in",
        "narration": "A lenda do homem amaldiçoado que se tornou o monstro mais aterrorizante dos rios do Piauí!",
        "duration": 3.0,
    ...
    # CENA 2: CONFLITO DIRETO / ORIGEM DA PRAGA (3.0s)
    {
        "scene_id": 2,
        "movement_type": "dolly_forward",
        "narration": "Após agredir a própria mãe, Crispim recebeu uma praga eterna e se transformou em uma besta aquática!",
        "duration": 3.0,
    ...
    # CENA 3: A MALDIÇÃO DO RIO PARNAÍBA (3.5s)
    {
        "scene_id": 3,
        "movement_type": "quick_pan_left",
        "narration": "Com a cabeça gigante em forma de cuia, ele habita as profundezas do Rio Parnaíba virando canoas de pescadores!",
        "duration": 3.5,
    ...
    # CENA 4: O MONUMENTO EM TERESINA (3.5s)
    {
        "scene_id": 4,
        "movement_type": "drone_reveal",
        "narration": "Sua lenda é tão forte que virou monumento no Parque Encontro dos Rios em Teresina-PI.",
        "duration": 3.5,
    ...
    # CENA 5: CALL TO ACTION VIRAL (3.0s)
    {
        ...
        "narration": "Já conhecia a aterrorizante lenda do Cabeça de Cuia? Comente e siga para mais segredos do Nordeste!",
        "duration": 3.0,
...
async def generate_fast_voice(text: str, out_path: str):
    ...
    for attempt in range(3):
        try:
            communicate = edge_tts.Communicate(text, "pt-BR-AntonioNeural", rate="+12%")
...
def produce_ultra_cabeca_de_cuia_short():
    topic_id = "short_cabeca_de_cuia"
    print(f"\n==========================================")
    print(f"[RE-RENDERIZAÇÃO DO SHORT CABEÇA DE CUIA (16s - REGRAS ROTA CALCULADA)]")
    print(f"==========================================")
    ...
    for sc in CABECA_DE_CUIA_SHORT_SCENES:
        ...
        fetch_cabeca_de_cuia_ai_image_8k(prompt_txt, scene_id, raw_img_path)
    ...
    temp_audio_file = str(output_dir / "temp_audio_cabeca_cuia.m4a")
    ...
    print(f"\n  🎉 [RE-RENDERIZAÇÃO DO SHORT CABEÇA DE CUIA CONCLUÍDA] DURAÇÃO TOTAL ({current_time:.1f}s): {master_path}")
    return master_path


def main():
    produce_ultra_cabeca_de_cuia_short()
