...
# EXPANSÃO PARA 18 SHORTS COMPLETOS DA LEGOLAND BILLUND:
LEGOLAND_18_SHORTS_DEFINITIONS = [
    {
    ...
    {
        "short_id": "short_legoland_13",
        "title": "O Aeroporto de Billund em Lego ✈️",
        "hook": "O AEROPORTO DE BILLUND EM LEGO! ✈️",
        "narration": "Olha a perfeição da réplica do Aeroporto Internacional de Billund na Miniland! Aviões, jatos e pistas de pouso em escala reduzida com aeronaves que taxiam de verdade!",
        "video_clips": [{"file": "WhatsApp Video 2026-08-12 at 18.49.46 (3).mp4", "start": 3.0, "dur": 14.5}]
    },
    {
        "short_id": "short_legoland_14",
        "title": "A História da Mini City de Lego 🏙️",
        "hook": "A HISTÓRIA DA MINI CITY DE LEGO! 🏙️",
        "narration": "A Mini City da Legoland nasceu em mil novecentos e sessenta e oito como o sonho de criar um mundo em miniatura! Construída com mais de vinte milhões de pecinhas Lego em escala um para vinte, essa cidade viva conta com veículos em movimento, canais reais e réplicas impressionantes!",
        "video_clips": [{"file": "WhatsApp Video 2026-08-12 at 18.49.46 (4).mp4", "start": 3.0, "dur": 14.5}]
    },
    {
        "short_id": "short_legoland_15",
        "title": "A Ilha do Tesouro Pirata 🏴‍☠️",
        "hook": "A ILHA DO TESOURO PIRATA! 🏴‍☠️",
        "narration": "Explore a Ilha do Tesouro na área dos Piratas! Navios naufragados, esqueletos e baús repletos de moedas de ouro montados com milhares de peças Lego!",
        "video_clips": [{"file": "WhatsApp Video 2026-08-12 at 18.49.45 (10).mp4", "start": 1.0, "dur": 11.0}]
    },
    {
        "short_id": "short_legoland_16",
        "title": "Os Animais e a Fazenda de Lego 🐄",
        "hook": "OS ANIMAIS DA FAZENDA DE LEGO! 🐄",
        "narration": "Olha que riqueza de detalhes nos animais e na fazenda de Lego! Vacas, cavalos e celeiros montados peça por peça em cenários rurais incríveis!",
        "video_clips": [
    ...
    },
    {
        ...
        "narration": "Nos dias quentes de verão na Dinamarca, a área aquática da Legoland garante a diversão da criançada com jatos d'água e brinquedos molhados interativos!",
        "video_clips": [{"file": "WhatsApp Video 2026-08-12 at 18.49.46 (1).mp4", "start": 0.5, "dur": 4.5}]
    },
    {
        "short_id": "short_legoland_18",
        "title": "Dicas para Visitar a Legoland 🇩🇰",
        "hook": "DICAS PARA VISITAR A LEGOLAND! 🇩🇰",
        "narration": "Vai viajar para a Dinamarca? A dica de ouro do Rota Calculada é reservar o dia inteiro para explorar a Legoland com calma e garantir os ingressos antecipados!",
        "video_clips": [{"file": "WhatsApp Video 2026-08-12 at 18.49.46 (8).mp4", "start": 30.0, "dur": 14.5}]
    }
]
...
def produce_all_legoland_videos(target_short_id=None):
    print("==========================================")
    print(f"[PRODUZINDO SUÍTE LEGOLAND: {'TODOS' if not target_short_id else target_short_id}]")
    print("==========================================")
    ...
    shorts_to_process = [s for s in LEGOLAND_18_SHORTS_DEFINITIONS if target_short_id is None or s["short_id"] == target_short_id or target_short_id in s["short_id"]]

    # --- 1. PRODUZIR OS SHORTS ---
    ...
    # --- 2. PRODUZIR DOCUMENTÁRIO MASTER ESTENDIDO (SE NÃO FOR SHORT ESPECÍFICO) ---
    if target_short_id is None:
        ...
        print(f"\n🎉 [SUÍTE EXPANDIDA DA LEGOLAND CONCLUÍDA] 18 Shorts + Documentário Master ({long_time:.1f}s / {long_time/60.0:.2f} min) -> {long_master_path}")
    else:
        print(f"\n🎉 [SHORT INDIVIDUAL CONCLUÍDO] {target_short_id}")

if __name__ == "__main__":
    target = sys.argv[1] if len(sys.argv) > 1 else None
    produce_all_legoland_videos(target)
