...
def fetch_ai_ruins_image_8k(prompt: str, out_path: Path) -> bool:
    """Gera uma IMAGEM DE IA 8K DAS RUÍNAS E DA CIDADE DESTRUIDA DO CALDEIRÃO."""
    ...
    seed_val = (int(time.time()) + 99887) % 999999
            ...
            with urllib.request.urlopen(req, timeout=18) as resp:
                ...
                if len(content) > 15000:
                    with open(out_path, "wb") as f:
                        f.write(content)
                    print(f"    ✓ [IMAGEM IA 8K DAS RUÍNAS DO CALDEIRÃO GERADA] '{prompt[:45]}...' ({out_path.name})")
                    return True
        except Exception:
            time.sleep(1.0)
    return False
...
def render_scene_clip_5_ruins(img_path: Path, narration_text: str, duration: float, movement_type: str, out_mp4_path: Path):
    """Renderiza o clipe MP4 24 FPS especificamente com a imagem das RUÍNAS DA CIDADE DESTRUIDA para a Cena 5."""
    ...
    for f_idx in range(total_frames):
        ...
        # Movimento Slow Push In em direção às ruínas
        scale = 1.0 + 0.15 * prog
        angle = 0.0
        dx, dy = 0.0, 0.02 * prog
    ...
    out_v.release()
    print(f"    ✓ [CLIPE CENA 5 DAS RUÍNAS RE-GERADO OK] ({duration:.1f}s)")
...
def insert_destroyed_city_scene():
    topic_id = "misterio_caldeirao_do_deserto"
    print(f"\n==========================================")
    print(f"[INSERINDO CENA DA CIDADE DESTRUIDA E RUÍNAS DO CALDEIRÃO]")
    print(f"==========================================")
    ...
    # 1. Gerar Imagem por IA 8K das Ruínas da Cidade Destruída para a Cena 5
    narration_5 = "Em 1937, o arraial foi cercado e desfeito, deixando para trás apenas as ruínas de pedra e a memória de um sonho no sertão."
    prompt_5_ruins = "Photorealistic 8k historical documentary photograph, emotional ruins of abandoned 1930s sertanejo village in Ceara, broken stone walls, burnt adobe structures, solitary wooden cross standing on dry ground at dramatic golden sunset, ARRI Alexa 65, Kodak Vision3, National Geographic style"
    
    raw_img_path = images_dir / "raw_scene_5.jpg"
    formatted_img_path = images_dir / "scene_5.png"
    scene_mp4 = output_dir / "scene_5.mp4"

    fetch_ai_ruins_image_8k(prompt_5_ruins, raw_img_path)
    format_photo_to_916_hd(raw_img_path, formatted_img_path)
    render_scene_clip_5_ruins(formatted_img_path, narration_5, 9.5, "dolly_forward", scene_mp4)
    ...
    all_scenes_info = [
        ...
        (5, "Em 1937, o arraial foi cercado e desfeita, deixando para trás apenas as ruínas de pedra e a memória de um sonho no sertão."),
    ...
    temp_audio_file = str(output_dir / "temp_audio_ruins.m4a")
    ...
    print(f"\n  🎉 [REINSERÇÃO DA CENA DAS RUÍNAS CONCLUÍDA] VÍDEO NOVO ({current_time:.1f}s): {master_path}")
    return master_path


def main():
    insert_destroyed_city_scene()
